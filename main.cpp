...
/// 
/// Reduced arrays are in reduced form
...
public:
    CriticalPointsInterpolator(Ms & ms, double T0, double rho0) : ms(ms) {
    ...
    auto interpolate_tilde(const double m) {
        if (m < ms[0] || m > ms[ms.size()-1]) {
            ...
            throw std::invalid_argument("How did I get here for m of " + std::to_string(m));
        }
    }
...
/// Trace away from the critical point down to a minimum temperature
class VLETracer {
public:
    ...
    auto interpolate_tilde(const double Ttilde_) {
        ...
        if (Ttilde_ > Ttilde[0]*(1+eps) || Ttilde_ < Ttilde[Ttilde.size() - 1]*(1-eps)) {
            ...
            for (auto i = 0; i < Ttilde.size() - 1; ++i) {
                if (Ttilde_ <= Ttilde[i]*(1+eps) && Ttilde_ >= Ttilde[i+1]*(1-eps)) {
                    return std::make_tuple(linterp(Ttilde, rhotildeL, i, Ttilde_), linterp(Ttilde, rhotildeV, i, Ttilde_));
                }
            }
            throw std::invalid_argument("How did I get here?");
        }
    }
...
int main(){
    ...
    auto Ms = Eigen::ArrayXd::LinSpaced(101, 1, 101);
