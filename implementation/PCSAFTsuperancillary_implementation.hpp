...
auto get_Ttilde_crit_min(double m){
    ...
    if (m < mmin){
        throw std::invalid_argument("Provided value of m of " + std::to_string(m)
         + " is less than min of "+ std::to_string(mmin));
    }
    if (m > mmax){
        throw std::invalid_argument("Provided value of m of " + std::to_string(m) 
        + " is greater than max of "+ std::to_string(mmax));
    }
...
auto PCSAFTsuperanc_rhoLV(double Ttilde, double m){
    ...
    if (Ttilde < Ttilde_min){
        throw std::invalid_argument("Provided value of Ttilde of " + std::to_string(Ttilde) 
        + " is less than min of "+ std::to_string(Ttilde_min));
    }
    if (Ttilde > Ttilde_crit){
        throw std::invalid_argument("Provided value of Ttilde of " + std::to_string(Ttilde) 
        + " is greater than max of "+ std::to_string(Ttilde_crit));
    }
